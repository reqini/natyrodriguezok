...
export default function QuienSoySection() {
  ...
  const getFotoPath = (foto: string) => `${import.meta.env.BASE_URL}${foto}`;
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="hidden md:grid grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-7 text-lg text-slate-700 leading-relaxed">
            <h3 className="text-3xl font-bold text-slate-900">Hola, soy Natalia</h3>
            <p>
              Mamá, creadora de contenido y apasionada por ayudar a las personas
              a mejorar su alimentación de una manera simple, real y sin complicaciones.
            </p>
            <p>
            ...
            {fotos.map((foto, i) => (
              <img
                ...
                className={`absolute rounded-3xl shadow-2xl object-cover transition-all duration-500 cursor-pointer
                  ...
                  ${active === i ? "scale-110 z-20" : "opacity-90 hover:scale-105"}
                `}
              />
            ))}
          </div>
        ...
        <div className="md:hidden">
          <div className="text-center space-y-5 text-lg text-slate-700 mb-10">
            <h3 className="text-2xl font-bold text-slate-900">Hola, soy Natalia</h3>
            <p>
          ...
          <div className="flex justify-center gap-6 mt-6">
            <button
              onClick={() => setActive(prev => prev === 0 ? fotos.length - 1 : prev - 1)}
              className="bg-white shadow-lg px-4 py-2 rounded-full"
            >
              ←
            </button>
            <button
              onClick={() => setActive(prev => prev === fotos.length - 1 ? 0 : prev + 1)}
              className="bg-white shadow-lg px-4 py-2 rounded-full"
            >
              →
            </button>
            ...
            {fotos.map((_, i) => (
              <div
                ...
                className={`w-2.5 h-2.5 rounded-full transition cursor-pointer
                  ${active === i ? "bg-pink-500 scale-125" : "bg-slate-300"}
                `}
              />
            ))}
          </div>
